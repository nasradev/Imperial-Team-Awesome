...
% get the centroids
s = regionprops(L,'centroid', 'orientation', 'BoundingBox', 'area');
...
%         
%         % create an array where the ratios between the sides of the
%         % bounding boxes will be stored
%         ratios = zeros(1,numTemp);
%         
%         % for all the regions in the tiny image
%         for j = 1:numTemp
%             % compute de bounding box
%             bbTemp = cat(1, sTemp.BoundingBox);
%             
%             % calculate ratio
%             ratios(j) = bbTemp(j,3) ./ bbTemp(j,4);
%         end
%         
%         % take the bigger
%         listIndex(i,5) = max(ratios);
%     end
% end
